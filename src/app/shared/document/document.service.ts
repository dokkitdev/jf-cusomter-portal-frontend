...
export class DocumentService {
  ...
  public search({
    ...
    const request = new DocumentPaginationRequest({
      ...filters,
      page,
      perPage,
      orderBy,
    ...
    return this.apiService
      ...
      >(this.endpoint, omitBy(classToPlain<DocumentPaginationRequest>(request), isUndefined))
      .pipe(
        map((response) =>
          plainToClassFromExist(new PaginationResponse<Document>(Document), response, { groups: [ClassGroup.MAIN] })
        )
      );
  }
  ...
  public get(id: number, relations?: Array<DocumentRelationType>): Observable<Document> {
    return this.apiService
      ...
      .pipe(map((response) => plainToClass(Document, response, { groups: [ClassGroup.MAIN] })));
  }
